...
void LoginConnection::OnNewConnection(std::shared_ptr<EQ::Net::DaybreakConnection> connection)
{
	...
	Log.OutF(Logs::General, Logs::HeadlessClient, "Connecting...");
}
...
void LoginConnection::OnStatusChangeActive(std::shared_ptr<EQ::Net::DaybreakConnection> conn, EQ::Net::DbProtocolStatus from, EQ::Net::DbProtocolStatus to)
{
	if (to == EQ::Net::StatusConnected) {
		Log.OutF(Logs::General, Logs::HeadlessClient, "Login connected.");
		SendSessionReady();
	}

	if (to == EQ::Net::StatusDisconnected) {
		Log.OutF(Logs::General, Logs::HeadlessClient, "Login connection lost, reconnecting.");
		m_key.clear();
		m_dbid = 0;
		m_connection.reset();
...
void LoginConnection::ProcessLoginResponse(const EQ::Net::Packet &p)
{
	...
	if (response_error > 101) {
		Log.OutF(Logs::General, Logs::HeadlessClient, "Error logging in response code: {0}", response_error);
		Kill();
	}
	else {
		...
		Log.OutF(Logs::General, Logs::HeadlessClient, "Logged in successfully with dbid {0} and key {1}", m_dbid, m_key);
		SendServerRequest();
	}
}
...
void LoginConnection::ProcessServerPacketList(const EQ::Net::Packet &p)
{
	...
	for (auto server : m_world_servers) {
		if (server.second.long_name.compare(m_server) == 0) {
			Log.OutF(Logs::General, Logs::HeadlessClient, "Found world server {0}, attempting to login.", m_server);
			SendPlayRequest(server.first);
			return;
		}
	}

	Log.OutF(Logs::General, Logs::HeadlessClient, "Got response from login server but could not find world server {0} disconnecting.", m_server);
	Kill();
}
