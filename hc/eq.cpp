...
EverQuest::EverQuest(const std::string &host, int port, const std::string &user, const std::string &pass, const std::string &server, const std::string &character)
{
	...
	EQ::Net::DNSLookup(m_host, port, false, [&](const std::string &addr) {
		if (addr.empty()) {
			Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Could not resolve address: {0}", m_host).c_str());
			return;
		}
		else {
			m_host = addr;
...
void EverQuest::LoginOnNewConnection(std::shared_ptr<EQ::Net::DaybreakConnection> connection)
{
	...
	Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, "Connecting...");
}
...
void EverQuest::LoginOnStatusChangeReconnectEnabled(std::shared_ptr<EQ::Net::DaybreakConnection> conn, EQ::Net::DbProtocolStatus from, EQ::Net::DbProtocolStatus to)
{
	if (to == EQ::Net::StatusConnected) {
		Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, "Login connected.");
		LoginSendSessionReady();
	}

	if (to == EQ::Net::StatusDisconnected) {
		Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, "Login connection lost before we got to world, reconnecting.");
		m_key.clear();
		m_dbid = 0;
...
void EverQuest::LoginProcessLoginResponse(const EQ::Net::Packet & p)
{
	...
	if (response_error > 101) {
		Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Error logging in response code: {0}", response_error).c_str());
		...
		Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Logged in successfully with dbid {0} and key {1}", m_dbid, m_key).c_str());
...
void EverQuest::LoginProcessServerPacketList(const EQ::Net::Packet & p)
{
	...
	for (auto server : m_world_servers) {
		if (server.second.long_name.compare(m_server) == 0) {
			Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Found world server {0}, attempting to login.", m_server).c_str());
			LoginSendPlayRequest(server.first);
			return;
		}
	}

	Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Got response from login server but could not find world server {0} disconnecting.", m_server).c_str());
	LoginDisableReconnect();
}
...
void EverQuest::LoginProcessServerPlayResponse(const EQ::Net::Packet &p)
{
	...
	if (allowed) {
		...
		Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Failed to login to server with message {0}", message).c_str());
		LoginDisableReconnect();
	}
}
...
void EverQuest::WorldOnNewConnection(std::shared_ptr<EQ::Net::DaybreakConnection> connection)
{
	...
	Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, "Connecting to world...");
}
...
{
	if (to == EQ::Net::StatusConnected) {
		Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, "World connected.");
		WorldSendClientAuth();
	}

	if (to == EQ::Net::StatusDisconnected) {
		Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, "World connection lost, reconnecting.");
...
void EverQuest::WorldOnPacketRecv(std::shared_ptr<EQ::Net::DaybreakConnection> conn, const EQ::Net::Packet & p)
{
	...
	case 0x00d2:
		...
		break;
	default:
		Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Unhandled opcode: {0:#x}", opcode).c_str());
		break;
	}
}
...
void EverQuest::WorldProcessCharacterSelect(const EQ::Net::Packet &p)
{
	...
	//Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("{0} characters", char_count).c_str());
	for (uint32_t i = 0; i < char_count; ++i) {
		...
		if (m_character.compare(name) == 0) {
			Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Found {0}, would attempt to login here.", m_character).c_str());
			WorldSendEnterWorld(m_character);
			return;
		}
	}

	Log.Out(Logs::General, Logs::HeadlessClient, __FILE__, __func__, __LINE__, fmt::format("Could not find {0}, cannot continue to login.", m_character).c_str());
}